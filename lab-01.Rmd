...
# Do not edit this code chunk!
visvisitors <- read_csv("data/UK-visitor-numbers.csv")
```
...
_How many 'O' Outside attractions are there in the `"Yorkshire and the Humber"` region that gives `"Members"` free admission, which had more than 100,000 visitors in 2022?_
...
Because many of the values for n_2021 are NA, can't do the calculations for median and mean when some values are strings and not numbers.
...
Suggests most of the values are lower than the mean, but there are some really large values that are outliers and make the mean higher.
...
```{r Ex_m}
visitors_with_nations %>%
  mutate(proportion = count(admission = "Free") / count(admission))
  
  
  
  
  
#count(admission = "Free") / (count(admission = "Free", admission = #"Charged", admission = "Member"))
